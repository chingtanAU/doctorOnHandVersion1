...
class DoctorHomePageController extends GetxController {
  ...
  final  Rx<List<BookingService>> allDoctrorMeetings= Rx<List<BookingService>>(List.empty(growable: true)) ;
  ...
  BookingService getEaliestMeeting(){
    ...
    final meetings= getUpcomingMeetings(allDoctrorMeetings.value, DateTime.now());
  ...
  Future<List<BookingService>?> fetchDoctorMeetings(String id) async{
    await meetingCrud.fetchDoctorMeetings(id).then((value) => allDoctrorMeetings.value=value);
  }
}
